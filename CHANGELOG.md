...
- **Reverts the `play_install_referrer` dependency added in 0.9.0, which cost
  this package its iOS and web platform tags on pub.dev.** The automatic
  referrer read worked. What it also did was relabel the package
  **Android** — pub.dev derives a package's platform support from its
  dependencies, and that plugin supports Android only. So the flagship
  Flutter SDK of a cross-platform push product told every iOS developer it
  does not support their platform. That is a worse outcome than four lines
  in an app, and it was not a cost anyone priced when the trade was made.

  `notibase_flutter` is pure Dart with one dependency again, and reports
  Android, iOS and web as before. Nothing else in 0.9.0 changed: the click
  id still travels through Google Play's referrer, and the server still
  reads it.

- **`setInstallReferrer()` is how an Android install gets attributed**, as
  it was in 0.8.1. Add
  [`play_install_referrer`](https://pub.dev/packages/play_install_referrer)
  to your own app and pass what it gives you, once, after `configure()`:
  ...
  The native Android SDK still reads it for you — it has no platform tags to
  lose — so this is Flutter and React Native only.

## 0.9.0

- **Android install attribution now happens by itself.** 0.8.1 added
  `setInstallReferrer()` and left the reading to you — which meant adding a
  plugin and remembering to call us, and an SDK that attributes installs only
  when the customer wires it up is an SDK that mostly does not.

  `configure()` now reads Google Play's install referrer itself. Nothing to
  add, nothing to call. Three attempts across three launches and then it
  stops: a device with no Play Services can never answer, and binding a
  service every launch for an answer that will not come is a cost with no
  upside — but one bad launch, offline or with Play still starting, must not
  lose the campaign for good.

  `setInstallReferrer()` stays public for an app that gets the string some
  other way, and is still a no-op for an organic install's
  `utm_source=google-play&utm_medium=organic`.

- **A second dependency, and what it costs.** This is the first time the
  package has needed native code: reading that API is not something Dart can
  do. `play_install_referrer` is Android-only and is never touched on any
  other platform, but "pure Dart, no platform channels" is no longer true of
  an Android build, and the description says so now rather than quietly
  keeping the old sentence. The trade was made on purpose — the alternative
  is attribution that silently works for the few customers who read the
  right paragraph.

- **The previous release named a discontinued package.** 0.8.1 pointed at
  `android_play_install_referrer`, which pub.dev has marked discontinued in
  favour of `play_install_referrer`. Nothing to do if you are upgrading —
  the SDK brings the right one.

## 0.8.1

- **`setInstallReferrer()` — Android store installs can finally be
  attributed.** They could not be, and the reason was invisible: the Play
  listing is a web page, and it drops every query parameter it does not
  recognise. So the `nb_click` a campaign link put on the URL never reached
  the app that got installed, and the server was left guessing from a
  24-hour window of clicks from the same IP — which misses whenever the
  browser and the app leave the phone on different addresses. On mobile
  that is most of the time.

  `referrer` is the one string Google Play carries across, and it comes back
  through the Install Referrer API on first launch. The redirect now puts
  the click id there, and this hands it to us. A campaign's own utm tags are
  kept alongside, with ours first so a truncation loses theirs.

  This package owns no platform channels, so it cannot read that API itself
  — the same reason it cannot ask for push permission or open a URL. Add
  [`play_install_referrer`](https://pub.dev/packages/play_install_referrer)
  and pass what it gives you:
  ...
  Safe on every launch, safe with null, and safe with an organic install's
  `utm_source=google-play&utm_medium=organic`, which carries nothing of ours.

  The native SDK does not depend on
  `com.android.installreferrer:installreferrer` either — the zero-dependency
  rule is what lets it drop into any app without an argument about
  transitive versions. Add it to your app and call
  `Notibase.setInstallReferrer(...)` with what it gives you.

- **And when it still does not match, the reason is now written down.**
  The server hashed the caller's IP, failed to find a click, and threw the
  hash away — so "they clicked the link and installed and nothing was
  recorded" could only ever be answered with a guess. Unattributed installs
  now keep what they presented and why it missed. Nothing to change in your
  app.
