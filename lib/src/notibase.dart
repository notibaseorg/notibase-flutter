...
///    pay nothing for the convenience. The count matters beyond tidiness:
///    pub.dev derives platform tags from dependencies, so one Android-only
///    plugin makes the whole package Android-only there.
...
class Notibase {
  ...
  ///
  /// This package reads no native API, so it cannot fetch the referrer
  /// itself — the same reason it cannot ask for push permission or open a
  /// URL. It briefly did, in 0.9.0, and that cost the package its iOS and
  /// web platform tags on pub.dev: the plugin that reads the referrer
  /// supports only Android, and pub.dev derives a package's platforms from
  /// its dependencies. Being told an iOS push SDK does not support iOS is a
  /// worse outcome than four lines in your app.
  ///
  /// Add
  /// [`play_install_referrer`](https://pub.dev/packages/play_install_referrer)
  /// and pass what it gives you, once, after [configure]:
  ///
  /// ```dart
