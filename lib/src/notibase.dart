...
///  - TWO dependencies, and each one buys something Dart cannot do:
///    shared_preferences for device-id persistence, and
///    play_install_referrer for the one string Google Play carries into an
///    install. The second is what makes Android campaign attribution work
///    without every customer wiring the same plugin themselves — the cost
///    is that this package is no longer pure Dart on Android, and that
///    trade was made deliberately. It is never touched on any other
///    platform. firebase_messaging is still NOT a dependency:
///    [attachFirebase] takes the objects your app already has and talks to
///    them structurally, so apps that get their token another way pay
...
class Notibase {
  ...
  /// How many times the Play install referrer has been asked for.
  static const _referrerTriesKey = 'nb_referrer_tries';
  ...
  static Future<void> configure(String clientKey,
    ...
    // Deliberately not awaited: this binds a Play service, and no app should
    // wait on our attribution to draw its first frame.
    unawaited(_readInstallReferrer());
  }

  /// Ask Google Play what campaign this install came from.
  ///
  /// The Play listing drops every query parameter it does not recognise, so
  /// the click id on the link a person tapped never reaches the app they
  /// install. `referrer` is the one string Play carries across, and this is
  /// where it is collected — automatically, because asking every customer to
  /// add a plugin and remember to call us is how attribution quietly does
  /// not work for most of them.
  ///
  /// Three attempts across three launches, then it stops. A phone with no
  /// Play Services can never answer, and binding a service on every launch
  /// for an answer that will not come is a cost with no upside — but one bad
  /// launch (offline, or Play still starting) must not lose the campaign for
  /// good, which a single attempt would.
  static Future<void> _readInstallReferrer() async {
    final prefs = _prefs;
    if (prefs == null) return;
    if (kIsWeb || !Platform.isAndroid) return;
    final tries = prefs.getInt(_referrerTriesKey) ?? 0;
    if (tries >= 3) return;
    await prefs.setInt(_referrerTriesKey, tries + 1);
    try {
      final details = await PlayInstallReferrer.installReferrer;
      // Read once and never again: the value does not change, and the id
      // inside it is acted on once whatever happens next.
      await prefs.setInt(_referrerTriesKey, 3);
      await setInstallReferrer(details.installReferrer);
    } catch (e) {
      // Thrown on iOS, and on an Android device without Play Services — an
      // ordinary state for a sideloaded or Huawei build, not an error worth
      // putting in somebody's console on every launch.
      debugPrint('[Notibase] install referrer unavailable (attempt ${tries + 1} of 3): $e');
    }
  }
  ...
  ///
  /// **You do not normally call this.** [configure] reads the referrer for
  /// you through `play_install_referrer`, which this package depends on for
  /// exactly that reason. It is public for an app that gets the string some
  /// other way — its own referrer plugin, a test, a platform this package
  /// does not cover.
  ///
