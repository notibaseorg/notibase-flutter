...
class NotibaseApi {
  ...
  ///
  /// A durable opt-out recorded against the token, not a flag on a row: it
  /// survives re-registration and reinstalls, and is lifted only from your
  /// own backend with a server key. That is the point of it, and the reason
  /// it is the wrong call for a sign-out — see [logout].
  Future<void> unsubscribe(String deviceId,
      {String reason = 'user_request'}) async {
    await _request('POST', '/v1/unsubscribe', {
      'device_id': deviceId,
      'reason': reason,
    });
  }
  ...
  ///
  /// The inverse of [identify]. The device stays registered and stays in
  /// the audience; it stops belonging to anybody, so campaigns aimed at a
  /// person no longer reach it and its inbox stops resolving.
  Future<void> logout(String deviceId) async {
    await _request('POST', '/v1/logout', {'device_id': deviceId});
  }
