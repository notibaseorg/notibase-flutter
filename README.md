...
<p align="center">
  Push notifications, in-app messages, an in-app inbox and campaign
  attribution for Flutter — <b>no code of our own runs natively</b>.<br>
...
Official Flutter SDK for [Notibase](https://notibase.com). Every line of it
is Dart, and it declares no platform channels of its own, so it adds nothing
to your build complexity and can never conflict with your Firebase setup.

Two dependencies, and each buys something Dart cannot do: `shared_preferences`
for device-id persistence, and `play_install_referrer` for the one string
Google Play carries into an install — which is what makes Android campaign
attribution work without you wiring anything. The second is Android-only and
is never touched on another platform.
...
Declaring no platform channels of its own means this package does not draw
