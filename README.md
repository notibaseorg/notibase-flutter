...
```dart
// Android store installs: the only thing that can carry a campaign click
// through Google Play. Add android_play_install_referrer and pass it on.
if (Platform.isAndroid) {
  final d = await AndroidPlayInstallReferrer.installReferrer;
  await Notibase.setInstallReferrer(d.installReferrer);
}
