...
void main() {
  ...
  test('an iOS token is read, not asked about', () {
    ...
    // 64 characters but not hexadecimal — the encoding is the test, not
    // the length.
    expect(Notibase.channelForToken('ios', 'z' * 64), 'fcm');

    // A simulator issues 80 bytes rather than a real device's 32. A rule
    // that asked for exactly 32 read this — verbatim from a customer's
    // log, a perfectly good Apple token — as Firebase's, and sent every
    // iOS notification to the wrong service.
    const simulator = '80524B3A1E11FE00AD7A1F43784ED0AD840B2839E2C17CB1B4653A5506CA968A'
        'B7AC5773ACFBFA0374973388E336132BCA2AF3DE7AFA60B8B5FDE8D237007D75'
        '3CFBAB9B7C02F3CFE763C4ECEBACBB0B';
    expect(simulator.length, 160);
    expect(Notibase.channelForToken('ios', simulator), 'apns');

    // Hex, but not a whole number of bytes, and shorter than anything
    // Apple issues — both far likelier to be a truncated paste.
    expect(Notibase.channelForToken('ios', apns + 'a'), 'fcm');
    expect(Notibase.channelForToken('ios', 'ab' * 31), 'fcm');
